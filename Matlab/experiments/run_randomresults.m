...
use_costcontigevolution = rand;          costevolution_incentivebalance = rand;
   ...
   % highPassFilter = floor((rand * 50)+50); %Hz
    ...
    name = sprintf('T:%.2fs BW:%d LPF:%d HPF:%d SFT:%d SS:%.1f@i%.1f SD:%.1f@i%.1f SY:%.1f@i%.1f CP:%.1f@i%.1f CF:%.1f@i%.1f SU:%.1f@i%.1f GA:%.1f@i%.1f CE:%.1f@i%.1f CONT:%d',...
           secondsPerTile, bandwidth,lowPassFilter, highPassFilter, solution_shift, ...
