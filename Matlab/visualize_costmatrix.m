...
'Cost Matrix @ T=%.2fs [SY:%.1f@i%.1f CP:%.1f@i%.1f CF:%.1f@i%.1f SU:%.1f@i%.1f GA:%.1f@i%.1f CE:%.1f@i%.1f] \n%s\nmean=%.1f heuristic=%.1f meandiff=%.1f White=ACTUAL Black=PREDICTED',...
   ...
   results.heuristic_score, cfg.solution_shift ));
xlabel('Tiles');
ylabel('Tiles');
colorbar;
...
draw_rectangles( [results.predictions_tilespace show.T .* cfg.secondsPerTile], 'k' );
