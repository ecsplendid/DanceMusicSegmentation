...
%it will be normalized at the end; it is easier to explain recursion without normalization
	...
	\begin{dmath*}
	\tilde C(f,t, \Omega) =	\sum_{i,j=f}^{t} \hat S_{ij}(\Omega)
	\end{dmath*}
where
	\begin{dmath*}
		\hat S_{ij}(\Omega)=	{ \left\{
			...
			\right.} 
	\end{dmath*} 
for all $i,j \in S$. The \textit{incentive bias} parameter $\Omega$ controls the balance of
positive and negative values. Direct computation using the definition
takes $O(TW^3)$ time. We can improve this to
$O(TW)$ by using the following recursion (assume that $f+1 \le t-1$, and note that the incentive bias parameter $\Omega$ has been temporarily removed for clarity):
	\begin{dmath*}
...
\[
\tilde C(f,t, \Omega) =  \sum_{i,j=f}^{t} {S}_{ij}
\]
